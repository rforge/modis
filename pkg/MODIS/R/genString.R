...
.genString <- function(x,local=TRUE,remote=TRUE,collection=NULL,localArcPath=.getDef('localArcPath')) {
	...
	if (missing(x)) {stop("'x' must be a file name or a product name!")}

	x <- basename(x)
	product <- getProduct(x)
	
	if (length(product)==1) {if(is.null(product)) { stop("'x' is not a valid 'product'")}}
...
if (length(strsplit(x,"\\.")[[1]])==1){ 
...
} else { # if x is a product name
	fname   <- MODIS:::.defineName(x) 
	product <- c(fname,unlist(product))
	...
	if (local) {
		...
		for (i in 1:length(tempString)){
			...
			#if  (length(s)==1) string[[i]] <- .getPart(product,s) 
		}
