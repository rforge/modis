    ...
    avFiles <- avFiles[doit]
        ...
        } else 
        {
            ...
            for (u in seq(along=avFiles))
            {
                ...
                path       <- MODIS:::.genString(product,what="metadata")
    
                if (
                    !file.exists(paste(avFiles[u],".xml",sep=""))
                    |
                    if (file.exists(paste(avFiles[u],".xml",sep="")))
                    { # check filesize of xml file! TODO  Needs improvement!!
                ...
                } else 
                {
                    ...
                    if (!sizes$isOK) 
                    {
                        if(!quiet)
                        {
                            ...
                            sizes$FileSize,", but should be: ",sizes$MetaSize,"\n",sep="")
                        }
                        ...
                        if (!sizes$isOK) 
                        {
                            if(!quiet)
                            {
                                ...
                                "\nFileSize is ",sizes$FileSize,", but should be: ",
                                sizes$MetaSize,"\n",sep="")
                            }
                        } else 
                        {
