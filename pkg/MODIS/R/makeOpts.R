makeOpts <- function(changeStruc=FALSE)
{
  ...
  # user settings
  if (file.exists(file.path("~/.MODIS_Opts.R",fsep="/")))
  {
    eval(parse(file.path("~/.MODIS_Opts.R",fsep="/")),envir=opts) # user options are overruling the defaults
  } else 
  {
    warning("Option file does not exists!\nOpen \'",file.path(find.package("MODIS"), "external","MODIS_Opts.R"),
            "\' with an editor, check/modify the values in that file and save it to: \'", 
            normalizePath("~/.MODIS_Opts.R","/",mustWork=FALSE),"\'\n
            If settings are fine as they are, just run the following command:\n\'
            file.copy('",file.path(find.package('MODIS'), 'external','MODIS_Opts.R'), "','",
            normalizePath("~/.MODIS_Opts.R","/", mustWork=FALSE), "')",sep="")
  }
  opt <- as.list(opts)	
